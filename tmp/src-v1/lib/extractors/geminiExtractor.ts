// src/lib/extractors GeminiExtractor.ts
import type {
  AssistantExtractor,
  LoginWaitOptions,
  LoginState,
...
export default class GeminiExtractor implements AssistantExtractor {
  ...
  private deriveRole(el: Element): 'user' | 'assistant' | 'system' | 'tool' {
    ...
    if (/\bassistant\b/i.test(ariaRole) || /\bassistant\b/i.test(cls)) return 'assistant';
    if (/\buser\b/i.test(ariaRole) || /\buser\b/i.test(cls)) return 'user';
